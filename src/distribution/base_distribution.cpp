...
double BaseDistribution::calc_exp_moment(const int order)
{
    std::complex<double> complex_order(0.0, -order);
    return calc_characteristic(complex_order).real();
}
...
double BaseDistribution::calc_exp_cos_moment(const int exp_order, const int cos_order)
{
    double result = 0.0;
    for(int k=0; k<=cos_order; ++k)
    {
        const auto coeff = static_cast<double>(nCr(cos_order, k));
        const std::complex<double> char_arg(2*k - cos_order, -exp_order);
        result += coeff * calc_characteristic(char_arg).real();
    }

    return result / std::pow(2.0, cos_order);
}

double BaseDistribution::calc_exp_sin_moment(const int exp_order, const int sin_order)
{
    std::complex<double> result(0.0, 0.0);
    for(int k=0; k<=sin_order; ++k)
    {
        const double coeff = static_cast<double>(nCr(sin_order, k)) * std::pow(-1.0, sin_order - k);
        const std::complex<double> char_arg(2*k - sin_order, -exp_order);
        result += coeff * calc_characteristic(char_arg);
    }
...
double BaseDistribution::calc_exp_x_cos_sin_moment(const int exp_order, const int x_order,
                                                   const int cos_order, const int sin_order)
{
    std::complex<double> result(0.0, 0.0);
    for(int k1=0; k1<=cos_order; ++k1)
    {
        for(int k2=0; k2<=sin_order; ++k2)
        {
            const auto cos_coeff = static_cast<double>(nCr(cos_order, k1));
            const auto sin_coeff = static_cast<double>(nCr(sin_order, k2)) * std::pow(-1.0, sin_order-k2);
            const std::complex<double> char_arg(2*(k1+k2) - cos_order - sin_order, -exp_order);
            result += cos_coeff * sin_coeff * calc_diff_characteristic(char_arg, x_order);
        }
    }
