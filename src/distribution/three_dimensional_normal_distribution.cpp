...
double ThreeDimensionalNormalDistribution::calc_xyz_cos_z_sin_z_moment(const int x_moment, const int y_moment, const int z_moment,
                                                                        const int cos_moment, const int sin_moment)
{
    if (!initialization_) {
        throw std::runtime_error("Need To Initialize two dimensional normal distribution");
        ...
        std::fabs(covariance_(0,1))<cov_threshold_)
    {
        ...
        return dist_x.calc_moment(x_moment) * dist_y.calc_moment(y_moment)
                * dist_z.calc_x_cos_sin_moment(z_moment, cos_moment, sin_moment);
    }
            ...
            for(int b1=0; b1<=b; ++b1) {
                for(int b2=0; b2<=b-b1; ++b2) {
                    for(int z1=0; z1<=z; ++z1) {
                        for(int z2=0; z2<=z-z1; ++z2) {
                            const double coeff_ncrxy = nCr(a, a1) * nCr(a-a1, a2) * nCr(b, b1) * nCr(b-b1, b2) * nCr(z, z1) * nCr(z-z1, z2);
                            const double coeff_txy = std::pow(t11, a1) * std::pow(t12, a2) * std::pow(t13, a-a1-a2)
                                                     * std::pow(t21, b1) * std::pow(t22, b2) * std::pow(t23, b-b1-b2)
                                                     * std::pow(t31, z1) * std::pow(t32, z2) * std::pow(t33, z-z1-z2)
                                                     / (std::pow(t31, a1+b1+z1) * std::pow(t32, a2+b2+z2) * std::pow(t33, a+b+z-a1-a2-b1-b2-z1-z2));
                            const double coeff_xy = coeff_ncrxy * coeff_txy;
                            for(int c1=0; c1<=c; ++c1) {
                                for(int c2=0; c2<=c1; ++c2) {
                                    for(int c3=0; c3<=c-c1; ++c3) {
                                        ...
                                        for(int s1=0; s1<=s; ++s1) {
                                            for(int s2=0; s2<=s1; ++s2) {
                                                for(int s3=0; s3<=s-s1; ++s3) {
                                                    const double coeff_sin = std::pow(-1, s2) * nCr(s, s1) * nCr(s1, s2) * nCr(s-s1, s3);
                                                    const double coeff = coeff_xy * coeff_cos * coeff_sin;
                                                    const double l1_moment = l1.calc_x_cos_sin_moment(a1+b1+z1, c+s-c1-s1, c1+s1);
                                                    const double l2_moment = l2.calc_x_cos_sin_moment(a2+b2+z2, c+s-c2-c3-s2-s3, c2+c3+s2+s3);
                                                    const double l3_moment = l3.calc_x_cos_sin_moment(a+b+z-(a1+a2+b1+b2+z1+z2), c+c2-c1-c3+s1+s3-s2, s+s2-s1-s3+c1-c2+c3);
                                                    result += coeff * l1_moment *l2_moment * l3_moment;
                                                }
                                            }
                                        }
