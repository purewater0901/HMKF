                       ...
                       const std::map<int, std::shared_ptr<BaseDistribution>>& measurement_noise_map)
{
    ...
    for(size_t i=0; i<augmented_size_; ++i) {
        ...
        const Eigen::VectorXd& system_noise = sigma_points_.col(i).segment(state_dim, system_noise_dim);
        const Eigen::VectorXd processed_state = vehicle_model_->propagate(curr_x, control_inputs, system_noise, dt);

        sigma_points_.col(i).head(state_dim) = processed_state;
    ...
    }
    {
        ...
        const Eigen::VectorXd& curr_x = sigma_points_.col(2*augmented_size_).head(state_dim);
        const Eigen::VectorXd& system_noise = sigma_points_.col(2*augmented_size_).segment(state_dim, system_noise_dim);
        const Eigen::VectorXd processed_state = vehicle_model_->propagate(curr_x, control_inputs, system_noise, dt);

        sigma_points_.col(2*augmented_size_).head(state_dim) = processed_state;
