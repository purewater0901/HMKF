...
KinematicVehicleUKF::KinematicVehicleUKF() : augmented_size_(9),
                                             alpha_squared_(1.0),
                                             beta_(0.0),
                                             kappa_(3.0),
                                  ...
                                  const std::map<int, std::shared_ptr<BaseDistribution>>& measurement_noise_map)
{
    ...
    augmented_mean(7) = dist_mvc->calc_mean();
    augmented_mean(8) = dist_myaw->calc_mean();
    ...
    augmented_cov(7, 7) = dist_mvc->calc_variance();
    augmented_cov(8, 8) = dist_myaw->calc_variance();
