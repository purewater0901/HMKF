...
int main() {
    ...
    std::shared_ptr<SimpleVehicleSquared::HighOrderMoments> hmkf_predicted_moments = nullptr;
    ...
    for(size_t odo_id = 0; odo_id < 10000; ++odo_id){
        ...
        if(measurement_id < measurement_time.size() && next_time - measurement_time.at(measurement_id) > 0.0) {
            ...
            while(measurement_id < measurement_time.size()) {
                ...
                if(hmkf_predicted_moments) {
                    hmkf_state_info = hmkf.update(hmkf_state_info, *hmkf_predicted_moments, y, {landmark.x, landmark.y}, measurement_noise_map);
